...
#include "ImportExport/LightmassScene.h"
	...
	FLightRayIntersection& ClosestIntersection) const
{
	...
	const int32 MaxNumIterativeIntersections = 100;
	int32 NumIterativeIntersections = 0;
	bool Dialate = Scene.IsPointInImportanceVolume(LightRay.Start, 1.0f);
	do 
	{
		FLightRay ClippedLightRay = LightRay;
		if (Dialate && Scene.NumImportanceVolumes == 1) {
			...
			FVector4 tocenter = center - ClippedLightRay.Start;
			FVector4 original = ClippedLightRay.Direction;
			float d = tocenter.Size3;
			//normalize vector length to 1
			tocenter = tocenter * (1 / d);
			//dialation is proportional to mass/ distance^2
			float k = mass / (d*d);

			FVector4 distortedVector = ((c - k)*original + (k * tocenter));

			ClippedLightRay.Direction = distortedVector / 300;
			ClippedLightRay.End = ClippedLightRay.Start + ClippedLightRay.Direction;
