...
public: 
		...
		, TraceFlags(InTraceFlags)
	{
		Length = 1.0f;
		if (true) {
			// using constants, cannot access members of Importance Volume
			//tweak these to adjust effects
			float mass = 300000000.0f;
			//c for proportion constant
			float c = 30000.0f;
			float horizon = 100.0f;
			FVector4 center(0.0, 0.0, 0.0, 0.0);
			//maths
			//direction vector from ray start to center
			FVector4 tocenter = Start * -1;// -Start.X, -Start.Y, -Start.Z, 0.0f);
			tocenter = tocenter / tocenter.Size3();
			float d = tocenter.Size3();
			//normalize vector length to 1
			tocenter = tocenter * (1 / tocenter.Size3());
			//dialation is proportional to mass/ distance^2
			float k = mass / ((d - horizon)*(d - horizon));//avoid divide by zero if horizon > 0

			FVector4 distortedVector = (((c-k)*Direction) + (k * tocenter));
			distortedVector = distortedVector / distortedVector.Size3();
			//check if signs on new direction and old direction are the same
			bool xsame = (distortedVector.X >= 0.0f) == (Direction.X >= 0.0f);
			bool ysame = (distortedVector.Y >= 0.0f) == (Direction.Y >= 0.0f);
			bool zsame = (distortedVector.Z >= 0.0f) == (Direction.Z >= 0.0f);
			//if all signs same, then there is a low probability of collision, so use new direction
			if (xsame && ysame && zsame) {
				Direction = distortedVector;
				End = Start + distortedVector;
			}
		}
	}
