...
static TAutoConsoleVariable<int> CVarCartoonizeSwitch(
	TEXT("r.Cartoonize.Switch"),
	0,
	TEXT("Switch for Cartoonize PostPreocess Pass \n")
	TEXT("  0 = OFF (default) \n")
	TEXT("  1 = ON\n"),
...
void FPostProcessing::Process(FRHICommandListImmediate& RHICmdList, const FViewInfo& View, TRefCountPtr<IPooledRenderTarget>& VelocityRT)
{
	...
	// so that the passes can register themselves to the graph
	{
		...
		if (AllowFullPostProcessing(View, FeatureLevel))
		{
			...
			// Insert Cartoonize Here
			if (CVarCartoonizeSwitch.GetValueOnRenderThread() == 1) {

				FRenderingCompositePass* CartoonizePass = Context.Graph.RegisterPass(new(FMemStack::Get()) FRCPassPostProcessCartoonize());

				CartoonizePass->SetInput(ePId_Input0, FRenderingCompositeOutputRef(Context.FinalOutput));
				CartoonizePass->SetInput(ePId_Input1, Context.SceneDepth);

				Context.FinalOutput = FRenderingCompositeOutputRef(CartoonizePass);
